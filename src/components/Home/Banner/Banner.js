...
const Banner = () => {
  ...
  return (
    <div className="container mx-auto max-h-[680px] shadow-md ">
      <div className="h-[240px] w-full md:h-[470px] lg:h-[680px] rounded relative overflow-hidden ">
        {/* arrow left */}
        <button
          onClick={prevSlider}
          className="absolute transform top-1/2 -translate-y-1/2 left-3 z-10 flex justify-center items-center bg-blue-500 rounded-full w-6 h-6 md:w-8 md:h-12"
        >
          <svg
        ...
        <button
          onClick={nextSlider}
          className="absolute transform top-1/2 -translate-y-1/2 z-10 right-3  flex justify-center items-center bg-blue-500 rounded-full w-6 h-6 md:w-8 md:h-12"
        >
          <svg
            className="w-4 h-4 md:w-6 md:h-6 icon"
            viewBox="0 0 1024 1024"
        ...
        <div className="flex justify-center items-center rounded-full z-50 absolute bottom-4 w-full gap-1">
          {sliderImages.map((_, inx) => (
            <button
              ...
              className={`rounded-full duration-300 bg-sky-500 ${
        ...
        >
          ...
          {sliderImages.map((slide, inx) => (
            <img
              key={slide}
              src={slide}
              className="min-w-full h-[240px] sm:h-96 md:h-[470px] lg:h-[680px] object-cover"
              alt={`Slider - ${inx + 1}`}
            />
          ))}
        </div>
