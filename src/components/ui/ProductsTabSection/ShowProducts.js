...
const ShowProducts = ({item}) => {
    const { title, image, author, ratings, stock, sold, price, discountPrice, discountAmount, category, review,height,width } = item
  return (
     ...
     />
      ...
      <div className="w-full flex-col items-center justify-center p-2 space-y-2">
        ...
        <div className=" flex items-center justify-evenly">
          <p> <span className='text-black'>Stock :</span> {stock}</p>
          <p><span className='text-black'> Sold :  </span>{sold}</p>
        </div>
        
        {
          height && width &&
        <div className=" flex items-center justify-evenly">
          <p> <span className='text-black'>Width :</span> {width}</p>
          <p><span className='text-black'>Height :  </span>{height}</p>
        </div>
